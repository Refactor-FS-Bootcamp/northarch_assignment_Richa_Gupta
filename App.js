...
function App() {
  ...
  const loadUsersData = async (start,end,increase,optType=null,filterOrSortValue) => {
    ...
    case "search":
      ...
      setSortValue("");
        ...
        return await axios.get(`http://localhost:5000/users?_sort=${value}&_order=asc&_start=${start}&_end=${end}`).then((response) => {
          setData(response.data);
          setCurrentPage(currentPage+increase);
        }).catch((err) => console.log(err));
        case "filter":
  ...
  const handleFilter=async(value)=>{
    ...
    // return await axios.get(`http://localhost:5000/users?status=${value}&_order=asc`).then((response) => {
    //   setData(response.data);
    // }).catch((err) => console.log(err));
  }
    ...
    if(currentPage === 0){
      ...
      return (
        <MDBPagination className="mb-0">
            ...
            <MDBPaginationItem>
              <MDBPaginationLink>
                {currentPage + 1}</MDBPaginationLink>
            </MDBPaginationItem>
  ...
  return (
    <MDBContainer>
      ...
      padding:"15px",
    maxWidth:"400px",
    alignContent:"center"}} className="d-flex input-group w-auto" onSubmit={handleSearch}>
     <input type="text" className="form-control" placeholder="search Name ..." value={value} onChange={(e)=>setValue(e.target.value)}/>
      ...
      <MDBRow>
        <MDBCol size="8">
             <h5>Sort By:</h5><select style={{width:"50%",borderRadius:"2px",heigth:"35px"}}
             onChange={handleSort} 
             value={sortValue}>
